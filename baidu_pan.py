...
class PersistDownloader:
    ...
    def download(self, url, filepath, label="", size_hint=0):
        ...
        if not total:
            total = int(resp0.getheader("Content-Length", 0)) or size_hint
        resp0.read()
        conn0.close()
        ...
        speed = downloaded / elapsed if elapsed > 0 else 0
        print(f"\r    ✓ {label} — {_fmt(downloaded)} ({_spd(speed)}, 重连{reconnects}次)        ")
        return downloaded
    ...
    def _dl_url(self, path):
        ...
        for app_id in [PCS_APP_ID, PAN_APP_ID]:
            try:
                ...
                # 排序：HTTP URL 排在 HTTPS 前面（qdall01的HTTP和HTTPS URL有不同的query参数）
                all_urls.sort(key=lambda x: 0 if x.startswith("http://") else 1)
            except Exception:
                continue
            ...
            try:
                ...
                for i, u in enumerate(dl_urls):
                    try:
                        ...
                        # 验证下载结果
                        fsize = os.path.getsize(lp) if os.path.exists(lp) else 0
                        if fsize > 1024:
                            ok_cnt[0] += 1; return
                        print(f"    {fn} 下载结果异常({fsize}B)，跳过")
                    except Exception as e:
                        print(f"    {fn} 下载异常: {e}")
                        continue
